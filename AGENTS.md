...
`terraform/cloudfront` holds the four CloudFront distributions across three workspaces (`shared`, `staging`, `prod`). It has no deploy workflow, so applies are manual and one workspace at a time. **Do not apply its `shared` workspace**: its plan carries drift that would strip rules from the edge WAFs and drop CORS headers from three cache keys. [Its README](terraform/cloudfront/README.md) records the detail.
