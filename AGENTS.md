...
The dead colour is reachable only through its own CloudFront distributions, at `mainnet.dead-cdn.prod.veworld.vechain.org` and `testnet.dead-cdn.prod.veworld.vechain.org`. The `*.dead.prod.veworld.vechain.org` records behind them are ALB origins that admit CloudFront traffic and nothing else, so pointing a suite at one times out. The `dead` workspace in `terraform/cloudfront` owns that front door.
...
`terraform/cloudfront` holds the four CloudFront distributions across three workspaces (`shared`, `staging`, `prod`) — two live, and two idle continuous-deployment canaries against the same prod origins. [deploy-shared-infra.yml](.github/workflows/deploy-shared-infra.yml) applies all three after the VPC stack, in that order, printing each plan to the job summary first. All three plan clean; the gate plans every workspace so you can see which a merge leaves dirty. [Its README](terraform/cloudfront/README.md) records the detail.
