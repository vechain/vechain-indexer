...
History and NFT reads exclude blacklisted collections with a `$lookup` into `nft_blacklist` (`NftBlacklistFilter` in `packages/api`), not a denormalised flag. The flag approach rewrote every row of a collection on each blacklist event and stalled indexing for minutes. The lookup is a point probe by `_id` into a collection of about 1.5K rows, so its cost scales with the rows scanned for a page, not with the size of the list. Keep the stage order `$match → $sort → $lookup → $match → $skip → $limit`: with the sort first the index supplies the order and probing stops once the page fills. Probing still covers every candidate scanned, so a wallet whose history is mostly blacklisted spam pays for the rows it skips, as the old residual `$ne` filter did. `findContractsByNftOwner` is the deliberate exception: it groups by contract before the lookup so each collection is probed once. The `NftBlacklistIndexer` owns that collection and starts at the blacklist contract's deployment block.
