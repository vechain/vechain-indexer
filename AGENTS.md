...
`make build` requires Go (>= 1.26) on the host because `build-indexer` depends on `build-thor-scheduler`, which compiles the Go co-process under `tools/thor-scheduler/`. The sub-Makefile auto-discovers Go on `PATH`, then under `~/sdk/go*/bin/go`, then `/opt/homebrew/bin/go`, then `/usr/local/go/bin/go`; set `GO=/path/to/go` to override. Docker-only flows (`make start`, `make build-image`, `make test-e2e`) build the binary inside `golang:1.26-alpine` and need no local Go. The integration test `packages/indexer/src/test/kotlin/org/vechain/indexer/validator/scheduler/ThorSchedulerProcessTest.kt` uses `assumeTrue` and silently skips when the binary is absent — run `make build-thor-scheduler` once before `make test-indexer` to exercise it.
