...
A child on an `align = false` edge resyncs alone: bump only its key. The indexers on its dependency chain, its parents and their other readers, restart at its start block and skip until they meet it, so do this on the dead colour only; since indexer-core 11.2.1 every other indexer keeps its own group at the head. A change to a parent's logic still needs every reader of the parent bumped with it, because their stored rows were derived from the parent's old values.
...
The only allowed exception is a narrow downstream-derived pattern with an explicit `.dependsOn(parent, align = false)` relationship. In that case, the downstream indexer may read the upstream schema only from a table that schema keeps unpruned, as of the block it is processing, and only when the dependency is one-way and the downstream rows are clearly derived from upstream data. A read of the parent's current row is not allowed: it only matches the child's block while both run in lockstep, which a resync of either breaks.
...
The **API** may join across Postgres schemas, because it owns no rollback unit: the `nft` and `history` reads anti-join `nft_blacklist.collection_state` inline rather than carrying a denormalised flag that a later state change would have to backfill. This is a reading rule for the API only; an indexer still writes and reads its own schema. The Stargate token reads take their delegation status from `delegation.state` the same way. The standing downstream readers are delegation, history and token_reward: they read `validator.cycle`, and token_reward also `delegation.state`, as of their own block, and each checks the parent has committed that block first.
...
"Indexer changed" is the image content hash differing from what live runs, or `terraform/api` differing from the release last applied there. That release is recorded in SSM at `/veworld/<colour>/deployed-release` after each application apply, because an unchanged image keeps its old tag and so cannot date the Terraform; without a marker the indexer's tag stands in. Deploying a changed indexer to live pauses indexing, so `target: live` is refused in that case. A running colour's indexers keep its data current, so it is never restored; a stopped or absent one is, unless `skip_restore` is set, in which case it indexes on from whatever its clusters hold. Misaligned networks stop the run: align them with a single-network DNS switch first.

**Confirmation is an environment gate.** The `confirm` job runs on the `prod-release-approval` environment, whose required reviewers see the plan as the job name ("Deploy v.1.2.3 to prod-green (dead), restoring from prod-blue snapshots first"). Approve to apply it; reject and nothing has changed, then re-dispatch with an explicit `target`. The environment's reviewers are configured in repository settings, not in the workflow: without them the gate does not pause. After approval the live colour is resolved from DNS again and the run stops if it moved during the wait, since the VPC apply would otherwise write the old colour back into Route53.
