...
@Repository
open class PostgresVetDelegatedByBlockRepository(
    private val jdbcTemplate: JdbcTemplate,
    private val objectMapper: ObjectMapper,
) : VetDelegatedByBlockRepository {

    private val tableName = "stargate_total_vet_delegated_by_block"

    override fun saveAll(records: Iterable<VetDelegatedByBlock>): Iterable<VetDelegatedByBlock> {
        ...
        jdbcTemplate.batchUpdate(
            ...
            INSERT INTO $tableName (
                ...
                day_total, week_total, month_total, year_total, total, by_level,
                total_nft_count, nft_count_by_level
            ...
            ON CONFLICT (block_number) DO UPDATE SET
                ...
                nft_count_by_level = EXCLUDED.nft_count_by_level
            """
                .trimIndent(),
            recordsList.map { record ->
                arrayOf(
                    ...
                    objectMapper.writeValueAsString(
                        record.byLevel.mapKeys { it.key.name }.mapValues { it.value.toString() }
                    ),
                    record.totalNftCount,
                    objectMapper.writeValueAsString(record.nftCountByLevel.mapKeys { it.key.name }),
                )
            },
        )
    ...
    private fun mapRow(rs: ResultSet): VetDelegatedByBlock {
        ...
        val byLevelMap: Map<String, String> = objectMapper.readValue(byLevelJson)
        val byLevel =
            byLevelMap
                .mapNotNull { (key, value) ->
                    try {
                        TokenLevel.valueOf(key) to BigInteger(value)
                    } catch (_: Exception) {
                        null
                    }
                }
        ...
        val nftCountByLevelJson = rs.getString("nft_count_by_level")
        val nftCountByLevelMap: Map<String, Long> = objectMapper.readValue(nftCountByLevelJson)
        val nftCountByLevel =
            nftCountByLevelMap
                .mapNotNull { (key, value) ->
                    try {
                        TokenLevel.valueOf(key) to value
                    } catch (_: Exception) {
                        null
                    }
                }
