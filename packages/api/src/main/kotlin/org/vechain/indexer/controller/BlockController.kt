...
open class BlockController(private val blockService: BlockService) {
    ...
    @Operation(
        summary =
            "Get a block by ID, number, 'best' for the latest block or 'finalized' for latest finalized block."
    )
    @ApiResponses(value = [ApiResponse(responseCode = "404", description = "Block not found")])
    @Parameter(
        `in` = ParameterIn.PATH,
        name = "revision",
        description =
            "block ID, number, 'best' for the latest block or 'finalized' for latest finalized block",
        required = true,
        example = "best"
    )
    open fun getBlock(@Revision @PathVariable revision: String): IndexedBlock {
        ...
        val block =
            when {
                ...
                HexUtils.isValidBlockID(normalisedRevision) ->
