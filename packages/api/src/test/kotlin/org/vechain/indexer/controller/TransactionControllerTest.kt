...
internal class TransactionControllerTest : AbstractIntegrationTest() {
    ...
    inner class TransactionsById {
        ...
        fun `get transaction by id with invalid id should return INVALID_REQUEST`() {
            mockMvc.get("$baseEndpoint/invalid_id").andExpect { status { isBadRequest() } }
        }
        ...
        fun `get transaction by id with valid endpoint should return OK`() {
            val result =
                mockMvc
                    .get(
                        "$baseEndpoint/0x0569d985aff6e073af33415f5ca4e848742cb483533015486dd96779c6e8251d?expanded=true"
                    )
    ...
    inner class OriginTransactions {
        ...
        fun `get transactions for bad address should return BAD_REQUEST`() {
            mockMvc.get("$baseEndpoint?origin=badAddress&size=$resultsPageSize").andExpect {
                status { isBadRequest() }
            }
        }
    ...
    inner class PaginatedOriginTransactions {
        ...
        fun `get txs by origin - pagination defaults`() {
            ...
            val result =
                mockMvc
                    .get("$baseEndpoint?origin=$origin&expanded=true")
                    .andExpect { status { isOk() } }
                    .andReturn()
        ...
        fun `get txs by origin - pagination with page only`() {
            ...
            val result =
                mockMvc
                    .get("$baseEndpoint?origin=$origin&page=$page&expanded=true")
                    .andExpect { status { isOk() } }
                    .andReturn()
        ...
        fun `get txs by origin - pagination with size only`() {
            ...
            val result =
                mockMvc
                    .get("$baseEndpoint?origin=$origin&size=$size&expanded=true")
                    .andExpect { status { isOk() } }
                    .andReturn()
        ...
        fun `get txs by origin - pagination with page & size`() {
            ...
            val result =
                mockMvc
                    .get("$baseEndpoint?origin=$origin&page=$page&size=$size&expanded=true")
                    .andExpect { status { isOk() } }
                    .andReturn()
    ...
    inner class DelegatedTransactions {
        ...
        fun `get DELEGATED transactions for bad address should return BAD_REQUEST`() {
            mockMvc.get("$baseEndpoint/delegated?delegator=badAddress").andExpect {
                status { isBadRequest() }
            }
        }
        ...
        fun `get DELEGATED transactions should return OKAY`() {
            val result =
                mockMvc
                    .get(
                        "$baseEndpoint/delegated?delegator=0xf077b491b355E64048cE21E3A6Fc4751eEeA77fa&expanded=true"
                    )
                    .andExpect { status { isOk() } }
                    .andReturn()
        ...
        fun `no hex prefix should be valid`() {
            val result =
                mockMvc
                    .get(
                        "$baseEndpoint/delegated?delegator=f077b491b355E64048cE21E3A6Fc4751eEeA77fa&expanded=true"
                    )
                    .andExpect { status { isOk() } }
                    .andReturn()
    ...
    inner class PaginatedDelegatedTransactions {
        ...
        fun `get delegated txs - pagination defaults`() {
            ...
            val result =
                mockMvc
                    .get("$baseEndpoint/delegated?delegator=$origin&expanded=true")
                    .andExpect { status { isOk() } }
                    .andReturn()
        ...
        fun `get delegated txs - pagination with page only`() {
            ...
            val result =
                mockMvc
                    .get("$baseEndpoint/delegated?delegator=$origin&page=$page&expanded=true")
                    .andExpect { status { isOk() } }
                    .andReturn()
        ...
        fun `get delegated txs - pagination with size only`() {
            ...
            val result =
                mockMvc
                    .get("$baseEndpoint/delegated?delegator=$origin&size=$size&expanded=true")
                    .andExpect { status { isOk() } }
                    .andReturn()
        ...
        fun `get delegated txs - pagination with page & size`() {
            ...
            val result =
                mockMvc
                    .get(
                        "$baseEndpoint/delegated?delegator=$origin&page=$page&size=$size&expanded=true"
                    )
                    .andExpect { status { isOk() } }
                    .andReturn()
