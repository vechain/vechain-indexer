...
open class NftHoldersByBlockConfig(private val indexerVersionService: IndexerVersionService) {
    @Value("\${indexer.version.stargate-nft-holders-by-block:1}") private var version: Int = 1

    @PostConstruct
    open fun initVersionCheck() {
        indexerVersionService.ensureTableExists(
            indexerName = IndexerNames.NFT_HOLDERS_BY_BLOCK,
            tableName = "nft_holders_by_block",
            schemaResource = "db/tables/nft_holders_by_block.sql",
            newVersion = version,
        )
    }
