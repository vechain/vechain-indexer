...
open class AppRoundActionSummaryConfig(private val indexerVersionService: IndexerVersionService) {
    @Value("\${indexer.version.b3tr-app-round-action-summary:1}") private var version: Int = 1

    @PostConstruct
    open fun initVersionCheck() {
        indexerVersionService.ensureTableExists(
            indexerName = IndexerNames.APP_ROUND_ACTION_SUMMARY,
            tableName = "b3tr_app_action_summaries_round",
            schemaResource = "db/tables/b3tr_app_action_summaries_round.sql",
            newVersion = version,
        )
    }
    ...
    open fun appRoundActionSummaryIndexer(
        ...
        appRoundActionSummaryPruner: PostgresPruner,
        ...
        IndexerFactory()
            .name(IndexerNames.APP_ROUND_ACTION_SUMMARY)
            .thorClient(thorClient)
            .processor(processor)
