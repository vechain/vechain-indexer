...
abstract class DebugIndexer(
    ...
    private tailrec suspend fun run() {
        try {
            ...
            // TODO test the wrong previous block from branch as well: bad 16734347L
            val block = getBlock()
    ...
    private suspend fun getBlock(): Block {
        if (currentBlockNumber == 16734347L) {
            if (i == 0) {
                i++
                return getFaultyBlock16734347()
            } else return getBlockFromChain(currentBlockNumber)
        }
        if (currentBlockNumber == 16734348L) {
            if (j == 0) {
                j++
                return getFaultyBlock16734348()
            } else return getBlockFromChain(currentBlockNumber)
        } else return getBlockFromChain(currentBlockNumber)
    }

    private fun getFaultyBlock16734348(): Block {
        return JsonUtils.mapper.readValue(
            DebugIndexer::class.java.getResource("/bad_block_16734348.json")!!.readText(),
            Block::class.java
        )
    }

    private fun getFaultyBlock16734347(): Block {
        return JsonUtils.mapper.readValue(
            DebugIndexer::class.java.getResource("/bad_block_16734347.json")!!.readText(),
            Block::class.java
        )
    }
