    ...
    override fun initCollection() {
        ...
        ensureIndexes(
            listOf(
                ...
                "navigator_fee_navigator_1_blockTimestamp_-1" to
                    Index()
                        .on(NavigatorFee::navigator.name, Sort.Direction.ASC)
                        .on(NavigatorFee::blockTimestamp.name, Sort.Direction.DESC),
                "navigator_fee_roundId_1_blockTimestamp_-1" to
                    Index()
                        .on(NavigatorFee::roundId.name, Sort.Direction.ASC)
                        .on(NavigatorFee::blockTimestamp.name, Sort.Direction.DESC),
            )
        )
    }
