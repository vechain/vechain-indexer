...
class StargateEventService(
    ...
    fun handleStargateEvents(
        ...
        validatorEvents.forEach { event ->
            handleValidatorExitRequested(event, latestTokenSnapshots, validatorSnapshots)
        }
        ...
        groupedEvents.forEach { (tokenId, tokenEvents) ->
            ...
            tokenEvents.forEach { event ->
                current = processEvent(event, tokenId, current, validatorSnapshots)
            }
    ...
    private fun processEvent(
        ...
        when (event.eventType) {
            ...
            "TokenBurned" -> handleTokenUnstaked(event, base)
            "Transfer" -> handleTokenTransfer(event, base)
            "DelegationInitiated" -> handleDelegate(event, base, validatorSnapshots)
            "DelegationExitRequested" -> handleDelegateExitRequest(event, base!!)
            "DelegationWithdrawn" -> handleExitDelegate(base)
            "TokenManagerAdded" -> handleManagerAdded(event, base)
            "TokenManagerRemoved" -> handleManagerRemoved(event, base)
            "MaturityPeriodBoosted" -> handleTokenBoosted(event, base)
            "BaseVTHORewardsClaimed",
            "DelegationRewardsClaimed" -> handleRewardsClaimed(event, base)
            else -> base
        }
    ...
    private fun handleManagerAdded(event: IndexedEvent, base: StargateToken?): StargateToken =
        base!!.copy(
    ...
    fun handleManagerRemoved(event: IndexedEvent, base: StargateToken?): StargateToken =
        base!!.copy(
            version = base.version + 1,
            blockId = event.blockId,
            blockNumber = event.blockNumber,
    ...
    // Rewards claimed event
    fun handleRewardsClaimed(event: IndexedEvent, base: StargateToken?): StargateToken {
    ...
    private fun handleExitDelegate(base: StargateToken?): StargateToken {
    ...
    private fun handleTokenBoosted(event: IndexedEvent, base: StargateToken?): StargateToken =
        base!!.copy(
            version = base.version + 1,
            blockId = event.blockId,
            blockNumber = event.blockNumber,
    ...
    private fun handleDelegateExitRequest(event: IndexedEvent, base: StargateToken): StargateToken {
            ...
            Status.QUEUED ->
                ...
                base.copy(
                    ...
                    delegationNextPeriod =
                        DelegationUtils.resolveNextCycleBlock(
    ...
    private fun handleTokenTransfer(event: IndexedEvent, base: StargateToken?): StargateToken =
        base!!.copy(
            version = base.version + 1,
            blockId = event.blockId,
            blockNumber = event.blockNumber,
    ...
    private fun handleTokenUnstaked(event: IndexedEvent, base: StargateToken?): StargateToken =
        base!!.copy(
            version = base.version + 1,
            blockId = event.blockId,
            blockNumber = event.blockNumber,
