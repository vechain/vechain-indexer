...
open class UserDailyActionSummaryConfig(private val indexerVersionService: IndexerVersionService) {
    @Value("\${indexer.version.b3tr-user-daily-action-summary:1}") private var version: Int = 1

    @PostConstruct
    open fun initVersionCheck() {
        indexerVersionService.ensureTableExists(
            indexerName = IndexerNames.USER_DAILY_ACTION_SUMMARY,
            tableName = "b3tr_user_action_summaries_daily",
            schemaResource = "db/tables/b3tr_user_action_summaries_daily.sql",
            newVersion = version,
        )
    }
    ...
    open fun userDailyActionSummaryIndexer(
        ...
        userDailyActionSummaryPruner: PostgresPruner,
