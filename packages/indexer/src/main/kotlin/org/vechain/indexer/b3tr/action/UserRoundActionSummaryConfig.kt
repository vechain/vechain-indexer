...
open class UserRoundActionSummaryConfig(private val indexerVersionService: IndexerVersionService) {
    @Value("\${indexer.version.b3tr-user-round-action-summary:1}") private var version: Int = 1

    @PostConstruct
    open fun initVersionCheck() {
        indexerVersionService.ensureTableExists(
            indexerName = IndexerNames.USER_ROUND_ACTION_SUMMARY,
            tableName = "b3tr_user_action_summaries_round",
            schemaResource = "db/tables/b3tr_user_action_summaries_round.sql",
            newVersion = version,
        )
    }
