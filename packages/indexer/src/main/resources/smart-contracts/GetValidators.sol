...
contract GetValidators {
  ...
  PriceFeedOracle private constant PRICE_FEED = PriceFeedOracle(0x299D83dfafF792058E0ee62482763ECBe0035c1A); // Update per network
  Stargate private constant STARGATE = Stargate(0x35Ce14062457ef7817e10BBC3815317F5A07d695); // Update per network
  ...
  returns (
    ...
    uint256[] memory nextPeriodDelegationStakes // nextPeriodValidatorWeights
  )
  {
    ...
    uint256[] memory nextPeriodDelegationStakes = new uint256[](count);
    ...
    for (uint i = 0; i < count; i++) {
      ...
      uint256 nextPeriodDelegationStake = STARGATE.getDelegatorsEffectiveStake(validatorId, compPeriods + 2);
      nextPeriodDelegationStakes[i] = nextPeriodDelegationStake;
    }
