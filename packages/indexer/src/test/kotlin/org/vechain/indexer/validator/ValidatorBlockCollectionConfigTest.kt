...
class ValidatorBlockCollectionConfigTest {
    ...
    @Test
    fun `initCollection creates status scoped block reward pagination index`() {
        ...
        every { mongoTemplate.collectionExists(ValidatorBlock::class.java) } returns true
        ...
        assertTrue(
            capturedIndexes.any {
                it.indexKeys["status"] == 1 &&
                    it.indexKeys["blockNumber"] == -1 &&
                    it.indexOptions["name"] == "status_1_blockNumber_-1"
            }
        )
    }
}
