...
class ProfiledTransactionService(
    ...
    fun processBlock(block: Block, events: List<IndexedEvent>): List<IndexedTransaction> =
        profiler.time("      TransactionService.processBlock") {
            if (block.transactions.isEmpty()) return@time emptyList()
    ...
    fun save(records: List<IndexedTransaction>) {
