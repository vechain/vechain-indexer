...
Subcommands:
    ...
    update-categories    Parse api-docs.json, regenerate category processors
    validate-categories  Compare generated category processors against api-docs.json
...
def to_datadog_path_query(path, url_attr):
    """Convert an OpenAPI path to a Datadog log query."""
    normalized_path = re.sub(r"\{[^/]+\}", "*", path)
    if "{" in path:
        return f"{url_attr}:{normalized_path}"
    return f'{url_attr}:"{normalized_path}"'
...
def build_api_category_categories(paths, url_attr):
    """Build the detailed API endpoint categories."""
    categories = [
        {"filter": {"query": to_datadog_path_query(path, url_attr)}, "name": path}
        for path in paths
    ]
    categories.append({"filter": {"query": f"{url_attr}:/*"}, "name": "Unknown"})
    return categories
...
def build_expected_main_category_processors(config):
    """Load OpenAPI spec and build expected category processors for the main pipeline."""
    ...
    return [
        {
            ...
            "categories": build_api_category_categories(
                extract_endpoint_paths(spec), ELB_URL_ATTR
            ),
        },
        {
...
def build_expected_waf_category_processors(config):
    """Load OpenAPI spec and build expected category processors for the WAF pipeline."""
    spec = load_openapi_spec()
    return [
        {
            "name": get_api_category_processor_name(config),
...
def update_pipeline_category_processors(path, processor_defs):
    """Update category processors in a pipeline file."""
    if not path.exists():
        print(f"Error: {path} not found. Run the matching get command first.", file=sys.stderr)
        sys.exit(1)
    ...
    for processor_def in processor_defs:
        ...
        print(f"{action} category processor '{processor_def['name']}' in {path.name}")
    ...
    for processor_def in processor_defs:
        print_categories(f"{path.name}:{processor_def['name']}", processor_def["categories"])

    print(f"\nSaved to {path}")


def validate_pipeline_category_processors(path, processor_defs):
    """Validate category processors in a pipeline file. Returns True when out of sync."""
    if not path.exists():
        print(f"Error: {path} not found. Run the matching get command first.", file=sys.stderr)
        sys.exit(1)
    ...
    for processor_def in processor_defs:
        ...
        out_of_sync = (
            report_category_diff(
                f"{path.name}:{processor_def['name']}",
...
def cmd_update_categories(args):
    """Parse api-docs.json, regenerate category processors in both pipeline files."""
    config = load_config()
    update_pipeline_category_processors(
        PIPELINE_PATH, build_expected_main_category_processors(config)
    )
    print()
    update_pipeline_category_processors(
        WAF_PIPELINE_PATH, build_expected_waf_category_processors(config)
    )
...
def cmd_validate_categories(args):
    """Compare generated category processors against api-docs.json. Exit 1 if out of sync."""
    config = load_config()
    out_of_sync = validate_pipeline_category_processors(
        PIPELINE_PATH, build_expected_main_category_processors(config)
    )
    out_of_sync = (
        validate_pipeline_category_processors(
            WAF_PIPELINE_PATH, build_expected_waf_category_processors(config)
        )
        or out_of_sync
    )
