...
module "ecs-lb-service-api" {
  ...
  source                     = "git::git@github.com:/vechain/terraform_infrastructure_modules.git//ecs-loadbalanced-webservice?ref=v.1.0.13"
...
module "ecs-backend-service" {
  ...
  # temporary filter to avoid modification of existing prod resources on deployment of blue/green
  for_each            = "${startswith(local.env.environment, "prod-") ? local.env.enabled_nets : {}}"
  source              = "git::git@github.com:/vechain/terraform_infrastructure_modules.git//ecs-backend-service?ref=v.1.0.13"
  vpc_id              = data.terraform_remote_state.vpc.outputs.vpc_id
  region              = local.env.region
