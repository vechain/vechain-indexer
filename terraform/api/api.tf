...
module "ecs-lb-service-api" {
  ...
  source                    = "git::git@github.com:/vechain/terraform_infrastructure_modules.git//ecs-loadbalanced-webservice?ref=v.1.0.48"
...
module "ecs-backend-service" {
  ...
  for_each         = local.env.enabled_nets
  source           = "git::git@github.com:/vechain/terraform_infrastructure_modules.git//ecs-backend-service?ref=v.1.0.48"
  vpc_id           = data.terraform_remote_state.vpc.outputs.vpc_id
  region           = local.env.region
