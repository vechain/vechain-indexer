locals {
  dashboard_body = {
    "widgets" : [
      for k, v in merge({for k, v in module.ecs-service : v.service_name => v.cluster_name}, {
        for k, v in module.ecs-lb-service : v.service_name => v.cluster_name
      }) : {
        "type" : "metric",
        "x" : 0,
        "y" : 0,
  ...
  api_id = sartswith(local.env.environment, "prod-") ? "" : aws_api_gateway_rest_api.currency_cache.name
