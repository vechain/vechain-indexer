...
# The CLOUDFRONT-scope WAFs are owned by terraform/vpc (PR #1519); this stack only
# reads their ARNs to associate them.
data "terraform_remote_state" "vpc" {
  ...
  config = {
    bucket = "veworld-indexer-terraform-state-prod"
    key    = "workspaces/prod/veworld-indexer-vpc.tfstate"
    region = "eu-west-1"
  }
}
