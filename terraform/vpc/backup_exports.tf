################################################################################
# Atlas Backup Export to S3 — kept alive until prod-blue has applied
#
# Nothing reads these any more. They outlive the rest of the Atlas teardown
# because Atlas refuses to delete an export bucket a cluster still exports to,
# and deploy.yml applies this stack before terraform/api, so blue's clusters
# cannot go first. They come out once blue has been deployed.
################################################################################
resource "aws_s3_bucket" "atlas_export_backups" {
  count  = local.env.environment == "prod" ? 1 : 0
  bucket = "veworld-indexer-atlas-backups"

  # Blue exports daily until it applies, so the expiry rule leaves objects behind.
  force_destroy = true
