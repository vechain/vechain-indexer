...
if [ "$target_kind" = dead ]; then
    if [ "$dead_state" = up ]; then
        ...
        restore_note="no; skipped by request. ${dead} indexes on from whatever its Postgres instances hold: a stale checkpoint for a stopped colour, nothing for a torn-down one"
    else
        restore=true
